...
@group(0)
@binding(0)
var<storage, read_write> gpu_data: array<SingleGpuThreadData>; // this is used as both input and output for convenience

@compute
@workgroup_size(1)
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    // TODO write search in terms of gpu_data[global_id.x] data.
    gpu_data[global_id.x].smallest_error = 123.456;
